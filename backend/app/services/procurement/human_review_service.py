...
class HumanReviewService:
    ...
    def sync_review_items_for_bid(cls, db: Session, bid_id: uuid.UUID) -> List[HumanReviewItem]:
        """
        Inspects upstream compliance results, verification records, and document extractions
        ...
        for cr in comp_results:
            ...
            if needs_review:
                ...
                # Determine severity and review type
                ...
                if existing:
                    # Update active system finding if open or in_review
        ...
        for existing in existing_items:
            ...
            if key not in active_source_keys and existing.status in (ReviewStatus.OPEN, ReviewStatus.IN_REVIEW):
    ...
    def get_review_queue(
        ...
        """
        Fetches the paginated human review queue for the authenticated Procurement Officer's organization.
        """
        _, profile, role = _verify_officer_access(db, user)
        ...
        if status_filter:
            stmt = stmt.where(HumanReviewItem.status == status_filter.upper())
        if severity:
        ...
        total_open = sum(1 for i in org_items if i.status == ReviewStatus.OPEN)
        critical_open = sum(1 for i in org_items if i.status in (ReviewStatus.OPEN, ReviewStatus.IN_REVIEW) and i.severity == ReviewSeverity.CRITICAL)
        in_review = sum(1 for i in org_items if i.status == ReviewStatus.IN_REVIEW)
        resolved_today = sum(1 for i in org_items if i.status == ReviewStatus.RESOLVED and i.resolved_at and i.resolved_at >= today_start)
        ...
        # 4. Sort: CRITICAL first, then by created_at desc
        severity_rank = {ReviewSeverity.CRITICAL: 0, ReviewSeverity.HIGH: 1, ReviewSeverity.MEDIUM: 2, ReviewSeverity.LOW: 3}
        filtered_items.sort(key=lambda x: (severity_rank.get(x.severity, 99), -(x.created_at.timestamp() if x.created_at else 0)))
        ...
        for item in paged_items:
            ...
            response_items.append(
                ReviewQueueItemResponse(
                    ...
                    review_type=ReviewTypeEnum(item.review_type),
                    severity=ReviewSeverityEnum(item.severity),
                    ...
                    resolution=ReviewResolutionEnum(item.resolution) if item.resolution else None,
    ...
    def get_review_detail(cls, db: Session, user: User, review_id: uuid.UUID) -> ReviewDetailResponse:
        ...
        if cr or vr:
            actual_section = ReviewActualEvidenceSection(
                ...
                verified_value=vr.verified_value if hasattr(vr, "verified_value") else None,
                match_status=vr.match_status if vr else None,
        ...
        # 7. AI Explanation Section (Advisory Only)
        ai_rec = db.scalars(
